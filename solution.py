...
class ASARProblem(search.Problem):
    ...
    def load(self, fh):
        ...
        self.initial = StateDict(legs, planes, 0, self.nodes) # initial state of the problem
    ...
    def actions(self, state):
        ...
        # actions is a list with [airplane, departure location, arrival location]
        actions = []
        #sate.planes keeps the information for each plane, i.e. in which airport each plane is and the time it leaves that airport
        for (airplane, info) in state.planes.items():
            # if the plane does not have yet a first leg / first airport defined all legs that comply with time constraint are ellectable
            if info == None:
                for leg in state.legs:
                    fligh_duration = self.l[leg][0]
                    arrival = state.time_sum(self.a[leg[0]][0],
                                             fligh_duration)  # check if the arrival time is after the closing time of the airport
                    if arrival <= self.a[leg[1]][1] and arrival >= self.a[leg[1]][
                        0]:  # arrival of aircraft > closing time arrival > opening time
                        actions.append([airplane, leg[0], leg[1]])
            else:
                flag = 0
                # check if it's possible to finish the all legs in the starting airport. if not, no actions are ellectable
                for leg in state.legs:
                    if state.schedule[airplane][1] == leg[1]:
                        flag = 1
               # ellect legs that comply with time constraints and where the departure airport matches with our current state airport
                if flag == 1:
                    for leg in state.legs:
                        if leg[0] == info[
                            1]:  # Check if the leg departure airport is equal to the state airport departure for that aricraft
                            if info[0] <= self.a[info[1]][1] and info[0] >= self.a[info[1]][
                                0]:  # departure is before closing time of the airport
                                fligh_duration = self.l[leg][0]
                                arrival = state.time_sum(info[0],
                                                         fligh_duration)  # check if the arrival time is after the closing time of the airport
                                if arrival <= self.a[leg[1]][1] and arrival >= self.a[leg[1]][
                                    0]:  # arrival of aircraft > closing time
                                    actions.append([airplane, leg[0], leg[1]])
        return actions
    ...
    def result(self, state, action):
        ...
        new_state = copy.deepcopy(state) # copies the current state info. new_state is the next generated state

        if len(new_state.legs) > 0:
            airplane_code = action[0] # name of the plane used in the current action

            airplane_type = self.p[airplane_code]  # class of the airplane. ex: a320
            protation_time = self.c[airplane_type] # rotation time for that type of plane

            # if the starting airport is not defined yet, we say the departure will happen when the airport opens and define the departure airport according to the leg
            ...
            new_state.planes[airplane_code][0] = new_state.time_sum(arrival,
                                                                    protation_time)  # new time of departure, airplane is added to dictionary
            ...
            link_cost = self.l[leg][index_cost] # profit to do that specific leg

            new_state.profit = new_state.profit + int(link_cost) # update of total profit
    ...
    def goal_test(self, state):
        ...
        # if we don't have more legs to do and the end airport corresponds to the star airport the state is a goal
        if not state.legs:
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        #cost is given by the accumulation of the inverse of the profit
    ...
    def heuristic(self, n):
        ...
        # sum the max profit of each leg for all legs. That sum give us the heuristic
        for leg in state.legs:
...
class StateDict(dict):
    ...
    def __lt__(self, other):
        t1 = self.schedule, self.legs, self.profit, self.planes
        t2 = other.schedule, other.legs, other.profit, other.planes
        return self.code < other.code

    def __eq__(self, other):
        return self.schedule == other.schedule and self.legs == other.legs and self.profit == other.profit and self.planes == other.planes
